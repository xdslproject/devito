...
#pragma omp for collapse(1) schedule(dynamic, chunk_size)
            ...
            {
#pragma omp simd aligned(nnz_sp_source_mask, save_src_u, source_id, sp_source_id, u : 16)
                for (int y = y_m; y <= y_M; y += 1)
                {
                    int sp_zi_M = nnz_sp_source_mask[x][y] - 1;
                    for (int sp_zi = sp_zi_m; sp_zi <= sp_zi_M; sp_zi += 1)
                    {
                        int zind = sp_source_id[x][y][sp_zi];
                        float r0 = save_src_u[time][source_id[x][y][zind]];
                        u[t0][x + 4][y + 4][zind + 4] += r0;
                    }
                }
            }
        }
