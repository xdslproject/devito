...
src.coordinates.data[1, :] = np.array(model.domain_size) * .18
...
# sparse_source_mask = np.zeros(((u.data[0].shape[0]),(u.data[0].shape[0]),(len(nzinds[0]))), dtype=np.uint8)
# sparse_source_id = np.zeros(((u.data[0].shape[0]),(u.data[0].shape[0]),(len(nzinds[0]))), dtype=np.uint8)
# sparse_source_mask_NNZ = np.zeros(((u.data[0].shape[0]),(u.data[0].shape[0])), dtype=np.uint8)




# sparse_source_mask[:,:,:]=np.where(source_mask[:,:,:] == 1)[2]

# z_slice_non_zeros = np.count_nonzero(source_mask,axis=2)
#for k in range(len(nzinds[0])) :
#    sparse_source_mask[nzinds[0][k],nzinds[1][k],k]=nzinds[2][k]
#    sparse_source_id[nzinds[0][k],nzinds[1][k],k]= source_id[nzinds[0][k],nzinds[1][k],nzinds[2][k]]
#    sparse_source_mask_NNZ



import pdb; pdb.set_trace()

ci = ConditionalDimension(name='ci', parent=source_mask.dimensions[2], condition=And(source_mask[source_mask.dimensions[0],source_mask.dimensions[1],source_mask.dimensions[2]] > 8, source_mask[source_mask.dimensions[0],source_mask.dimensions[1],source_mask.dimensions[2]] < 10, evaluate=False))
...
op = Operator([Eq(sparse_source_mask, sparse_source_mask + 1)]).apply()
import pdb; pdb.set_trace()
