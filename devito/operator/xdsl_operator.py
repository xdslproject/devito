...
XDSL_CPU_PIPELINE = lambda nb_tiled_dims, collapse: f'"stencil-shape-inference,convert-stencil-to-ll-mlir{{collapse={collapse} {"tile-sizes="+",".join(["64"]*nb_tiled_dims) if nb_tiled_dims > 0 else ""}}},printf-to-llvm"'
...
XDSL_MPI_PIPELINE = lambda decomp, nb_tiled_dims, collapse: f'"dmp-decompose{decomp},canonicalize-dmp,convert-stencil-to-ll-mlir{{collapse={collapse} {"tile-sizes="+",".join(["64"]*nb_tiled_dims) if nb_tiled_dims > 0 else ""}}},dmp-to-mpi{{mpi_init=false}},lower-mpi,printf-to-llvm"'
...
class XDSLOperator(Operator):
    ...
    def _jit_compile(self):
        ...
        with self._profiler.timer_on('jit-compile'):
            ...
            prob_dim = len(list(filter(lambda s : str(s) in ["x", "y", "z"], self.dimensions)))
            to_tile = 0 if prob_dim == 2 else prob_dim-1
            collapse = prob_dim-1

            xdsl_pipeline = XDSL_CPU_PIPELINE(to_tile, collapse)
