...
def blocking(clusters, options):
    ...
    # Wavefront TB requires at least 2 "blocklevels".
        ...
        for c in clusters:
            ...
            if c.properties[d] == {SEQUENTIAL, AFFINE}:
                if seq_dims:
                    ...
                    intervals = c.ispace
                    sub_iterators = c.ispace.sub_iterators
            ...
            # Skew intervals for SKEWABLE dimensions, interchange loops, update properties
                ...
                exprs = xreplace_indices(c.exprs, {d: d - skew_dim})  # Skew expressions

            # Rebuild clusters with new intervals and ispace
            intervals = IntervalGroup(intervals, relations)
            ispace = IterationSpace(intervals, sub_iterators, c.ispace.directions)
            ...
            if i.dim is d and i.dim is skew_dim:
                ...
                for j in c.ispace.sub_iterators[d]:
                    sub_iters.append(j.func(offset=i.dim/INT(sf) + j.offset -
                                     skew_dim) if j.is_Modulo else j)
