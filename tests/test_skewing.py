...
class TestCodeGen(object):
    ...
    '''
    Test code generation with blocking, skewing, combinations thereof,
    ...
    @pytest.mark.parametrize('expr', ('Eq(u.forward, u + 1)',
                                      ...
                                      'Eq(u.forward, u.dz2)'))
    def test_timeblocking_codegen(self, expr):
        ...
        op = Operator(eqn, opt=('advanced', {'skewing': True, 'blocktime': True,
                                'blockinner': True}))
        op.apply(time_M=5)
        ...
        assert len(iters) == 8
        assert_structure(op, ['time0_blk0tx0_blk0y0_blk0z0_blk0xyz'])

        assert iters[0].symbolic_min == iters[0].dim.symbolic_min
        assert iters[0].symbolic_max == iters[0].dim.symbolic_max

        assert iters[1].symbolic_min == iters[1].dim.symbolic_min
        assert iters[1].symbolic_max == MIN(iters[1].dim.symbolic_max,
                                            iters[0].symbolic_max)

        assert iters[2].symbolic_min == iters[1].dim + iters[2].dim.symbolic_min
        assert iters[2].symbolic_max == iters[1].dim + iters[2].dim.symbolic_max

        assert iters[3].symbolic_min == iters[1].dim + iters[3].dim.symbolic_min
        assert iters[3].symbolic_max == iters[1].dim + iters[3].dim.symbolic_max

        assert iters[4].symbolic_min == iters[1].dim + iters[4].dim.symbolic_min
        assert iters[4].symbolic_max == iters[1].dim + iters[4].dim.symbolic_max

        assert iters[5].symbolic_min == iters[5].dim.symbolic_min
        assert iters[5].symbolic_max == MIN(iters[5].dim.symbolic_max,
                                            iters[2].symbolic_max)

        assert iters[6].symbolic_min == iters[6].dim.symbolic_min
        assert iters[6].symbolic_max == MIN(iters[6].dim.symbolic_max,
                                            iters[3].symbolic_max)

        assert iters[7].symbolic_min == iters[7].dim.symbolic_min
        assert iters[7].symbolic_max == MIN(iters[7].dim.symbolic_max,
                                            iters[4].symbolic_max)


class TestNumericalCorrectness(object):
    '''
    Test numerical corectness of operators with skewing, timeblocking
    '''
    def test_correctness(self):
        ...
        assert len([i for i in iters if i.dim.is_Time]) == 2

        assert_structure(op1, ['tx0_blk0y0_blk0xyz'])
        assert_structure(op2, ['time0_blk0tx0_blk0y0_blk0x0_blk1y0_blk1xyz'])
        assert_structure(op3, ['time0_blk0tx0_blk0y0_blk0x0_blk1y0_blk1x0_blk2'
                               'y0_blk2xyz'])
        assert_structure(op4, ['time0_blk0tx0_blk0y0_blk0x0_blk1y0_blk1x0_blk2y0_blk2'
                               'x0_blk3y0_blk3xyz'])
    ...
    def test_correctness_II(self):
        ...
        u.data[:] = init_value
        op2 = Operator(eq0, opt=('advanced', {'openmp': True, 'blocktime': True}))
        ...
        assert_structure(op1, ['tx0_blk0y0_blk0xyz'])
        assert_structure(op2, ['time0_blk0tx0_blk0y0_blk0xyz'])

    @pytest.mark.parametrize('so', [4, 8, 12])
    @pytest.mark.parametrize('shape',
                             [((2, 2, 2)), ((2, 2, 2)),  # Corner cases
                              ((13, 17, 6)), ((2, 7, 17))])
    @pytest.mark.parametrize('nt', [1, 5, 10])
    def test_correctness_III(self, so, shape, nt):
        ...
        assert_structure(op1, ['time0_blk0tx0_blk0y0_blk0x0_blk1y0_blk1xyz'])

    @pytest.mark.parametrize('so', [4, 8, 16])
    @pytest.mark.parametrize('nt', [7, 10, 15])
    @pytest.mark.parametrize('shape',
                             [((13, 3, 15)), ((12, 15, 25)), ((13, 17, 11))])
    def test_correctness_IV(self, so, shape, nt):
        nx, ny, nz = shape
        nu = .5
        dx = 2. / (nx - 1)
        dy = 2. / (ny - 1)
    ...
    @pytest.mark.parametrize('nt', [2, 24, 41])
    @pytest.mark.parametrize('shape',
                             [((35, 29, 16)), ((12, 16, 45)),
                              ((3, 28, 14)), ((43, 35, 22))])
    ...
    def test_correctness_VI(self, shape, nt, so1, so2):
        ...
        op2 = Operator(eqns, opt=('advanced', {'openmp': True, 'blocktime': True}))
        op2.apply(time_M=nt)
